    ...
    for epoch in range(config.get('epochs', 10)):
        ...
        with torch.no_grad():
            for images, labels in val_loader:
                ...
                labels = labels.to(device)
                outputs = model(images)
                probs = torch.softmax(outputs, dim=1)[:, 1].cpu().numpy() if outputs.shape[1] > 1 else outputs[:,0].cpu().numpy()
        ...
        print(f"Fold {fold_id} Epoch {epoch+1}: Train Acc={acc:.4f}, Val Acc={val_acc:.4f}, F1={val_f1:.4f}, AUC={val_auc}")
        ...
        with open(os.path.join(config.get('log_dir','./logs'), f'fold{fold_id}_report_epoch{epoch+1}.txt'), 'w', encoding='utf-8') as f:
            f.write(report_text)
        ...
        try:
            ...
            for i in range(sample_count):
                # 取 val_dataset 中的样本
                img_path, _ = val_subset[i][0], val_subset[i][1]
                # val_subset returns (tensor,label) so we rebuild a PIL image from tensor
                pil_img = Image.fromarray((val_subset[i][0].permute(1,2,0).numpy()*255).astype('uint8'))
                inp = val_subset[i][0].unsqueeze(0).to(device)
        ...
        # WandB logging
        if use_wandb:
            logd = {'epoch': epoch+1, 'train_loss': total_loss, 'train_acc': acc, 'val_loss': val_loss, 'val_acc': val_acc, 'val_f1': val_f1}
            if val_auc is not None:
...
if k_folds is None or k_folds <= 1:
    ...
    train_loader = DataLoader(datasets.ImageFolder(train_path, transform=transform), batch_size=config.get('batch_size', 16), shuffle=True)
    val_loader = DataLoader(datasets.ImageFolder(val_path, transform=transform), batch_size=config.get('batch_size', 16))
